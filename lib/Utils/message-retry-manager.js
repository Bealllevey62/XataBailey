...
export class MessageRetryManager {
  constructor(logger, maxMsgRetryCount) {
    ...
    this.sessionRecreateHistory = new LRUCache({
      max: 50,
      maxAge: RECREATE_SESSION_TIMEOUT * 2
    });
  ...
  getRecentMessage(to, id) {
    return this.recentMessagesMap.get(`${to}:${id}`);
  }
  ...
  incrementRetryCount(messageId) {
    const prev = this.retryCounters.get(messageId) || 0;
    const count = prev + 1;
  ...
  markRetrySuccess(messageId) {
    ...
    this.retryCounters.del && this.retryCounters.del(messageId);
